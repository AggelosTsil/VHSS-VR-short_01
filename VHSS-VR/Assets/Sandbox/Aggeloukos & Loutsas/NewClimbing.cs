    ...
    void Update()
    {
        ...
        if (GrabRight.IsPressed()) {
            if (GrabRight.IsPressed() && GrabLeft.IsPressed()) {
                ...
                //RIGHT hand grabbing
                TravelPoint.Set(CurrentRHPosition.z - PreviousRHPosition.z, -CurrentRHPosition.y + PreviousRHPosition.y, -CurrentRHPosition.x + PreviousRHPosition.x);
                Debug.Log("<color=yellow>Right hand</color>");
            }
        }else if (GrabLeft.IsPressed()) {
            //LEFT hand grabbing
            TravelPoint.Set(CurrentLHPosition.z - PreviousLHPosition.z, -CurrentLHPosition.y + PreviousLHPosition.y, -CurrentLHPosition.x + PreviousLHPosition.x);
